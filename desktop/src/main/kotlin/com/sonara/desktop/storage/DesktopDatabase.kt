...
class DesktopDatabase {
    ...
    fun clearHistory() {
        getConnection().use { conn ->
            conn.createStatement().use { it.executeUpdate("DELETE FROM history") }
        }
    }
    ...
    fun createPlaylist(title: String, description: String = ""): Playlist {
        ...
        return Playlist(id = id, title = title, description = description)
    }
    ...
    fun removeTrackFromPlaylist(playlistId: String, trackId: String) {
        getConnection().use { conn ->
            val sql = "DELETE FROM playlist_tracks WHERE playlist_id = ? AND track_id = ?"
            conn.prepareStatement(sql).use { ps ->
                ps.setString(1, playlistId)
